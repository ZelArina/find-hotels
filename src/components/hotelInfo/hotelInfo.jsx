...
const HotelInfo = ({onCounterLiked, onSetHotelId, data, dataAPI, date, favLike, heart, favId}) => {
    const [like, setLike] = useState(favLike);
    ...
    useEffect(() => {
        if(dataAPI){
            setNumberDay(data[2]);
            setStars(dataAPI.stars);
            setName(dataAPI.hotelName);
            setPrice(Math.round(dataAPI.priceFrom));
        }
    },[dataAPI])
    ...
    useEffect(() => {
        if (+numberDay === 1){
            setCountDay(`${numberDay} день`);
        }else if(+numberDay > 1 && +numberDay < 5){
            setCountDay(`${numberDay} дня`);
        }else{
            setCountDay(`${numberDay} дней`);
        }
    }, [numberDay])
    ...
    return(
        <div className="favHotel">
            <div className="firstLine">
                <p>{name}</p>
                <button className={like?"heart active":"heart"}
                        onClick={onLiked}/>
            </div>
